...
class AndroidARView implements PlatformARView {
  ...
  Widget build(
    ...
    // PATCH: was a plain `AndroidView`, which puts the view in a Flutter
    // *virtual display*. A virtual display always reports rotation 0, so on an
    // activity locked to landscape (rotation 1) the ARSceneView renders the
    // camera feed 90 degrees off. Hybrid composition attaches the real view to
    // the activity's window, so it sees the activity's true display rotation.
    // See PATCHES.md.
