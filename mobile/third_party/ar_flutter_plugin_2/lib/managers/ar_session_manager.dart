...
class ARSessionManager {
  ...
  /// Receives hand-tracking ticks while hand tracking is enabled
  /// (see [setHandTracking])
  void Function(HandGestureFrame frame)? onHandGesture;
  ...
  /// Enables or disables camera-based hand tracking on the platform side.
  /// Returns false if hand tracking is unsupported on this device (e.g.
  /// iOS < 14 or the landmark model failed to load).
  Future<bool> setHandTracking(bool enabled) async {
    try {
      final supported = await _channel
          .invokeMethod<bool>('setHandTracking', {'enabled': enabled});
    ...
    try {
      switch (call.method) {
        ...
        case 'onPlaneDetected':
          ...
          break;
        case 'onHandGesture':
          if (onHandGesture != null) {
            onHandGesture!(HandGestureFrame.fromMap(
