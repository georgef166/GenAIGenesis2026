...
class IosARView: NSObject, FlutterPlatformView, ARSCNViewDelegate, UIGestureRecognizerDelegate, ARSessionDelegate {
    ...
    // Patch 6: hand tracking (stored as Any so the property is usable below iOS 14)
    private var _handTracker: Any? = nil
    @available(iOS 14.0, *)
    private var handTracker: HandTracker? {
        get { _handTracker as? HandTracker }
        set { _handTracker = newValue }
    }
    ...
    func onSessionMethodCalled(_ call :FlutterMethodCall, _ result:FlutterResult) {
        ...
        switch call.method {
            ...
            case "setHandTracking":
                // Patch 6: toggle camera-based hand tracking
                let enabled = arguments?["enabled"] as? Bool ?? false
                if #available(iOS 14.0, *) {
                    if enabled {
                        if handTracker == nil {
                            handTracker = HandTracker { [weak self] payload in
                                self?.sessionManagerChannel.invokeMethod("onHandGesture", arguments: payload)
                            }
                        }
                    } else {
                        handTracker = nil
    ...
    func session(_ session: ARSession, didUpdate frame: ARFrame) {
        ...
        if #available(iOS 14.0, *), let tracker = handTracker {
            let orientation = sceneView.window?.windowScene?.interfaceOrientation ?? .landscapeRight
            tracker.maybeDetect(frame: frame,
