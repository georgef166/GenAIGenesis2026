...
class ArView(
    ...
    // Patch 7: per-node factor that normalizes the raw model to 1 m, folded into
    // every applied transform so Dart scale values are sizes in meters.
    private val nodeUnitScales = mutableMapOf<String, Float>()
    ...
    private val onSessionMethodCall =
        MethodChannel.MethodCallHandler { call, result ->
            when (call.method) {
                ...
                "setHandTracking" -> handleSetHandTracking(call, result)
    ...
    private val onObjectMethodCall =
        MethodChannel.MethodCallHandler { call, result ->
            when (call.method) {
                ...
                // PATCH: ARObjectManager.onInitialize() calls "init" on this
                // channel, but upstream never handled it, so every session
                // threw MissingPluginException(No implementation found for
                // method init on channel arobjects_N). See PATCHES.md.
    ...
    init {
        sceneView = ARSceneView(
            // PATCH: was `viewContext`. SceneView resolves the display (and so
            // the camera's display rotation) from this context. A non-activity
            // context reports the display in the device's natural orientation,
            // which renders the camera feed 90 degrees off in an activity that
            // is locked to landscape. See PATCHES.md.
            context = activity,
    ...
    private suspend fun buildModelNode(nodeData: Map<String, Any>): ModelNode? {
        ...
        // Patch 7: upstream passed matrix[0] as scaleToUnits, which normalized the
        // model's largest dimension to scale.x meters and dropped the node's
        // position and rotation entirely. Instead, normalize every model to 1 m
        // (scaleToUnits = 1), remember that per-node normalization factor, and
        // apply the full transformation matrix with the scale multiplied by it.
        // Result: a Dart-side scale of S renders the model S meters across its
        // largest dimension, consistently at build time and on every subsequent
        // transformationChanged update.
        return try {
            sceneView.modelLoader.loadModelInstance(fileLocation)?.let { modelInstance ->
                object : ModelNode(
                    ...
                    val unitScale = scale.x
                    name?.let { nodeUnitScales[it] = unitScale }
                    applyTransformationList(this, transformation, unitScale)
                }
            } ?: run {
                null
            }
    ...
    private fun handleRemoveNode(
        ...
        try {
            ...
            nodesMap[nodeName]?.let { node ->
                ...
                nodeUnitScales.remove(nodeName)
    ...
    // Patch 7: decompose a column-major 16-element matrix and apply it to the node,
    // multiplying the matrix scale by the node's 1m-normalization factor.
    private fun applyTransformationList(
        node: ModelNode,
        ...
        node.transform(
            ...
            rotation = SceneRotation(
                ...
                y = kotlin.math.atan2(
                    -transform[2].toFloat(),
                    kotlin.math.sqrt(
                        transform[6].toFloat() * transform[6].toFloat() +
            ...
            scale = SceneScale(
                x = kotlin.math.sqrt((transform[0] * transform[0] + transform[1] * transform[1] + transform[2] * transform[2]).toFloat()) * unitScale,
                y = kotlin.math.sqrt((transform[4] * transform[4] + transform[5] * transform[5] + transform[6] * transform[6]).toFloat()) * unitScale,
                z = kotlin.math.sqrt((transform[8] * transform[8] + transform[9] * transform[9] + transform[10] * transform[10]).toFloat()) * unitScale,
            ),
        )
    }
    ...
    try {
        // Patch 4: programmatic transforms from Dart (node.transform/position/scale setters)
        // must apply regardless of native gesture flags; iOS applies them unconditionally.
        run {
            ...
            nodesMap[name]?.let { node ->
                newTransformation?.let { transform ->
                    ...
                    applyTransformationList(node, transform, nodeUnitScales[name] ?: 1.0f)
                    result.success(null)
    ...
    // Patch 5: serialize a Pose as a column-major 4x4 matrix (16 doubles), matching the
    // iOS implementation and the Dart-side MatrixConverter (Matrix4.fromList).
    private fun serializePose(pose: Pose): List<Double> {
    ...
    private fun handleGetCameraPose(result: MethodChannel.Result) {
        try {
            val cameraPose = lastCameraPose ?: sceneView.session?.update()?.camera?.pose
            if (cameraPose != null) {
    ...
    private fun handleGetAnchorPose(
        ...
        try {
            ...
            // Patch 5: resolve local anchors by the name Dart sends (anchorNodesMap key);
            // fall back to cloud anchors for backward compatibility.
    ...
    // Patch 6: camera-based hand tracking (MediaPipe Hand Landmarker) streaming distilled
    // pinch gestures to Dart via the session channel ("onHandGesture").
    private fun handleSetHandTracking(
        call: MethodCall,
        result: MethodChannel.Result,
    ) {
        try {
            val enabled = call.argument<Boolean>("enabled") ?: false
            Log.i(TAG, "setHandTracking(enabled=$enabled) invoked")
            if (enabled) {
                if (handTracker == null) {
                    handTracker = HandTracker(viewContext) { payload ->
                        mainScope.launch {
                            sessionChannel.invokeMethod("onHandGesture", payload)
                        }
                    }
                }
                val started = handTracker?.start() == true
                Log.i(TAG, "HandTracker start -> $started")
            ...
            Log.e(TAG, "Failed to toggle hand tracking", e)
            result.success(false)
        }
    }
